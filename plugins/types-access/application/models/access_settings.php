...
class Settings {
	 ...
	 * @return bool
	 */
	function check_if_post_type_managed( $status, $post_type ) {
	 ...
	 * @return mixed
	 */
	function optimize_settings( $settings ) {//print_r($settings);exit;
	...
	public function wpcf_access_role_to_level_map() {
		...
		if ( false === $map ) {
			...
			$default_roles = $access_capabilities->get_default_roles();
	 ...
	 * @return bool
	 */
	public function is_object_valid( $type, $data ) {
		...
		if ( ! isset( $data ) || empty( $data ) || ! isset( $data['labels'] ) ) {
			...
			$plural = ( isset( $data['labels']['menu_name'] )
				&& $data['labels']['name']
				!= $data['labels']['menu_name'] ) ? $data['labels']['menu_name'] : $data['labels']['name'];
