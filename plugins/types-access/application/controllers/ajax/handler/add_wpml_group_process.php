	...
	function process_call( $arguments ) {
		...
		if ( isset( $settings_access[$post_type]['permissions']['read']['roles'] ) ) {
			...
			foreach ( $ordered_roles as $role => $roles_data ) {
				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'read', $roles_data ) ) {
					$read[] = $role;
				}

				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'edit_posts', $roles_data ) ) {
					$edit_own[] = $role;
				}

				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'edit_others_posts', $roles_data ) ) {
					$edit_any[] = $role;
				}

				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'delete_posts', $roles_data ) ) {
					$delete_own[] = $role;
				}

				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'delete_others_posts', $roles_data ) ) {
					$delete_any[] = $role;
				}

				if ( Access_Admin_Edit::toolset_access_check_for_cap( 'edit_published_posts', $roles_data ) ) {
					$publish[] = $role;
				}
			}
		}
