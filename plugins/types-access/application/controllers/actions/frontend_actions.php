...
class FrontendActions {
	 ...
	 * @return bool
	 */
	function toolset_access_load_layout_archive_is_assigned() {
	 ...
	 * @return mixed
	 */
	function toolset_access_load_layout_archive() {
	 ...
	 * @return bool
	 */
	function is_layout_custom_error() {
		global $wp_query;
		$post_type_object = $wp_query->get_queried_object();
	 ...
	 * @param $template
	 */
	function toolset_access_error_php_template( $template ) {
		global $post;
	...
	public function toolset_access_error_template_layout( $content ) {
		...
		if ( false === $template_info ) {
			...
			$template_info = $frontend->get_custom_error( $post_id );
	 ...
	 * Return layout id
	 */
	public static function toolset_access_load_layout() {
	 ...
	 * @param $query
	 */
	function toolset_access_exclude_selected_post_from_single( $query ) {
		if ( ! is_admin() && $query->is_main_query() ) {
			$custom_errors = CustomErrors::get_instance();
			$post_id = $custom_errors->wpcf_access_get_current_page();
