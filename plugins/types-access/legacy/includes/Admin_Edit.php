 ...
 */
public static function wpcf_access_admin_edit_access( ) {
	$output = '';
...
public static function otg_access_get_permission_table_for_taxonomies() {
	...
	$output .= '<p class="toolset-access-tab-info">
		...
		<a href="https://toolset.com/documentation/user-guides/setting-access-control/?utm_source=access&utm_medium=admin-tabs&utm_term=Access%20Control%20for%20Taxonomies&utm_campaign=access#taxonomy" title="'. __('Access Control for Taxonomy ', 'wpcf-access') .'" target="_blank">'. __('Access Control for Taxonomy', 'wpcf-access') .'</a>
	</p>';
...
public static function otg_access_get_permission_table_for_third_party( $current_tab = 'third-party' ) {
	...
	if ( $current_tab == 'types-fields' ) {
		$output .= '<p class="toolset-access-tab-info">
			'. __( 'Expand each of these field groups to set their access control.', 'wpcf-access' ) .
                   '<br> <a href="https://toolset.com/documentation/user-guides/access-control-for-user-fields/?utm_source=access&utm_medium=admin-tabs&utm_term=Access%20Control%20for%20Fields&utm_campaign=access" title="'. __('Access Control for Fields', 'wpcf-access') .'" target="_blank">'. __('Access Control for Fields', 'wpcf-access') .'</a>
		</p>';
	}
	if ( $current_tab == 'cred-forms' ) {
		$output .= '<p class="toolset-access-tab-info">
			' . __('Expand each of these groups of forms (for posts and for users) to set their access control on the front-end.', 'wpcf-access') .
                   '<br> <a href="https://toolset.com/documentation/user-guides/access-control-for-cred-forms/?utm_source=access&utm_medium=admin-tabs&utm_term=Access%20Control%20for%20Toolset%20Forms&utm_campaign=access" title="' . esc_attr( __('Access Control for Toolset Forms', 'wpcf-access') ) .'" target="_blank">'. __('Access Control for Toolset Forms', 'wpcf-access') .'</a>
		</p>';
	}
		...
		foreach ( $groups as $group ) {
			...
			foreach ( $caps_filter as $cap_slug => $cap ) {
				...
				$saved_data[ $cap['cap_id'] ] =
						...
						$third_party[ $area['id'] ][ $group['id'] ]['permissions'][ $cap['cap_id'] ] : array( 'roles' => $access_roles->get_roles_by_role( '', $cap['default_role'] ) );
			}
...
public static function otg_access_get_permission_table_for_wpml() {
	...
	if ( apply_filters( 'toolset_access_filter_is_wpml_installed', false ) ) {
		...
		$group_output .= '<p class="toolset-access-align-right">
				<button " style="background-image: url('.ICL_PLUGIN_URL . '/res/img/icon_adv.png'.')" class="button button-large button-secondary wpcf-add-new-access-group wpcf-add-new-wpml-group js-wpcf-add-new-wpml-group js-wpcf-add-new-wpml-group-placeholder">'
...
public static function wpcf_access_admin_roles_dropdown( $roles, $name, $data = array(), $dummy = false, $enabled = true, $exclude = array() ) {
    $access_settings = \OTGS\Toolset\Access\Models\Settings::get_instance();
    $default_roles = $access_settings->wpcf_get_default_roles();
    $output = '';
...
public static function wpcf_access_permissions_table($roles, $permissions, $settings,
	...
	$output .= '<table class="wpcf-access-table js-otgs-table-sticky-header js-access-table widefat fixed striped">';
	...
	$output .= '<thead>';
		...
		foreach ($settings as $permission_slug => $data){
			...
			if (
				...
				$output .= '<span class="toolset-access-capability-tip js-otgs-popover-tooltip" data-tippy-content="'
					. $data['help_tip'] . '">'
		...
		foreach ( $settings as $permission_slug => $data ){
			...
			if ( $permission_slug == 'read'  && $role != 'administrator' && $id != 'attachment' ) {
				...
				if ( isset($current_archive_custom_errors[$role]) && !empty($current_archive_custom_errors[$role]) && isset($type_data['has_archive']) && $type_data['has_archive'] == 1  ) {
					...
					$archive_error_value_value = isset( $current_archive_custom_errors_value[$role] ) ? $current_archive_custom_errors_value[$role] : 'default_error';
					if ( $archive_error_type_value == 'default_error' ) {
						$archive_text = __('Display: \'No posts found\'','wpcf-access');
					} elseif ( $archive_error_type_value == 'error_ct' ) {
						$archive_text = __('View Archive', 'wpcf-access').': '. self::toolset_access_get_view_name($archive_error_value_value);
