...
WPViews.MapsAddressAutocomplete = function( $ ) {
	...
	const AUTOCOMPLETE_SETTINGS = {
		...
		open: function() {
			$( this ).data( 'uiAutocomplete' ).menu.element.addClass( 'toolset-maps-address-autocomplete-ui-menu' );
		},
	};
	...
	self.initMapPreview = function( counter, latLng ) {
		...
		// Sometimes Azure Maps get the container size too eagerly, and if it changes before they actually render, their
		// size might be off. This seems to fix it.
		renderedMap.addEventListener( 'render', function() {
			renderedMap.resize();
		} );
