				...
				'conditional_display' => array(
					'fields' => array(
						'information' => array(
							...
							'content'   => sprintf(
								__(
									'In order to get visitor location information, this shortcode %1$smust%2$s be inside the [wpv-geolocation] shortcode. To learn more, visit the %3$srelated documentation page%4$s.',
									'toolset-maps'
								),
								'<strong>',
								'</strong>',
						...
						'display' => array(
							...
							'type'              => 'select',
							'options'           => array(
								'inside'  => __( 'when inside the distance', 'toolset-maps' ),
								'outside' => __( 'when outside the distance', 'toolset-maps' ),
							),
							'defaultValue'      => $conditional_display_defaults['display'],
							'required'          => true,
							// Note: do not fix IDE thinking that the string is SQL code with /* @lang text */, because
							// it messes up string translation (it does not get picked up by the string scanner at all).
							'description'       => __(
								"Select if the content is displayed when visitor's location is inside or outside of the distance radius from the central location.",
								'toolset-maps'
							),
						),
						'content' => array(
