...
class Toolset_Addon_Maps_Common {
	...
	function __construct() {
		...
		self::$stored_options = get_option( self::SETTINGS_OPTION_NAME, array() );
	...
	function set_options() {
		update_option( self::SETTINGS_OPTION_NAME, self::$stored_options );
	}
...
HTML;
	...
	function display_admin_notices() {
        ...
        <div class="message notice notice-warning">
            <p>
				...
				<?php echo sprintf(
					__(
						'<strong>You need an API key</strong> to use Toolset Maps. Find more information in %1$sour documentation%2$s and visit the %3$sToolset Maps settings page%4$s to choose an API and enter the key.',
						'toolset-maps'
					),
	...
	function render_api_key_options( $saved_options ) {
		...
		<div class="js-wpv-map-plugin-form">
			...
			<p>
				...
				echo sprintf(
					__(
						'A Google Maps API key is %3$srequired%4$s to use Toolset Maps. Refer to %1$sour documentation%2$s if you do not know how to get one.',
						'toolset-maps'
					),
					'<a href="' . esc_attr( $link ) . '" target="_blank">',
					'</a>',
	...
	/**
	 * All API getters use throttling, and so all can emit this message.
	 *
	 * @since 2.0
	 *
	 * @return string
	 */
	private static function get_api_throttling_reached_message() {
		return __("API call limit reached. These coordinates will have to be fetched another time.", 'toolset-maps');
	}
	 ...
	 * @return boolean
	 */
	public static function are_coordinates_migrated() {
		return get_option( self::ARE_COORDINATES_MIGRATED_OPTION );
	}

	/**
	 * Transition method to get coordinates from new cache, if they are migrated.
	 *
	 * When we remove support for old cache storage, this code will move to get_coordinates().
	 *
	 * @since 2.0
	 *
	 * @param $address
	...
	private static function get_coordinates_new( $address, $skip_cache=false ) {
		...
		if ( ! $skip_cache ) {
			...
			if ( array_key_exists( $address, $coordinates_set ) ) {
				return $coordinates_set[ $address ];
			}
		}

		// If address is passed as {lat,lng} just convert to right format and return, but save to cache, because
		// distance filtering and ordering uses address cache directly.
		if (
			strpos( $address, '{' ) === 0
			...
			if ( count( $address_components ) === 2 ) {
				...
				if (
					...
					self::update_cached_coordinates( $address, $address, $address_lat, $address_lon );
		...
		// Get address coordinates from the API used
		$api_used = self::get_saved_option( 'api_used' );

		if ( self::API_GOOGLE === $api_used ) {
			$coordinates = self::get_coordinates_google( $address );
		} elseif ( self::API_AZURE === $api_used ) {
			$coordinates = self::get_coordinates_azure( $address );
		} else {
			return sprintf(
		...
		// Update cache if coordinates received from API
		if ( is_array( $coordinates ) ) {
			self::update_cached_coordinates(
				$address,
				$coordinates['address'],
				$coordinates['lat'],
				$coordinates['lon']
			...
			return $coordinates; // This is probably an error message as a string.
		}
	}

	/**
	 * Returns cached coordinates either from RAM or database table.
	 *
	 * @since 2.0
	 *
	 * @return array
	 */
	public static function get_cached_coordinates() {
		if ( self::$coordinates_cache === null ) {
			self::$coordinates_cache = self::load_cached_coordinates();
		}
		return self::$coordinates_cache;
	}

	/**
	 * Loads coordinate cache from database
	 *
	 * @return array
	 */
	private static function load_cached_coordinates() {
		global $wpdb;

		$table_name = $wpdb->prefix . CreateDatabaseTable::TABLE_NAME;
		$sql = "SELECT address_passed, ST_X(point) as lon, ST_Y(point) as lat FROM $table_name";
		$coordinate_cache = [];

		foreach( $wpdb->get_results( $sql, ARRAY_A ) as $row ) {
			$coordinate_cache[ $row['address_passed'] ] = [
				'lon' => $row['lon'],
				'lat' => $row['lat'],
			];
		}
	...
	/**
	 * Updates cached coordinates in RAM and database table.
	 *
	 * @since 2.0
	 *
	 * @param string $address_passed
	 * @param string $address Address canonical name, as returned by APIs. Not kept in RAM, but saved in database.
	 * @param float $lat
	 * @param float $lon
	 */
	public static function update_cached_coordinates( $address_passed, $address, $lat, $lon ) {
		global $wpdb;

		self::$coordinates_cache[ $address_passed ] = [
			'lon' => $lon,
			'lat' => $lat,
		];

		$table_name = $wpdb->prefix . CreateDatabaseTable::TABLE_NAME;
		$sql = "INSERT INTO $table_name ( `address_passed`, `address`, `point` ) "
			. "VALUES ( %s, %s, ST_PointFromText('POINT(%f %f)') )";
		$args = [ $address_passed, $address, $lon, $lat ];

		$wpdb->query( $wpdb->prepare( $sql, $args ) );
	}

	/**
	 * Removes address from RAM (if loaded) and database table.
	 *
	 * @since 2.0
	 *
	 * @param string $address
	 */
	public static function delete_cached_coordinates( $address ) {
		self::get_cached_coordinates(); // Make sure they're loaded, so the check on next line doesn't fail.
		if ( array_key_exists( $address, self::get_cached_coordinates() ) ) {
			unset( self::$coordinates_cache[ $address ] );

			global $wpdb;

			$table_name = $wpdb->prefix . CreateDatabaseTable::TABLE_NAME;
			$sql = "DELETE FROM $table_name WHERE $table_name.address_passed = %s LIMIT 1";
			$args = [ $address ];
	...
	static function get_coordinates( $address, $skip_cache=false ) {
		if ( self::are_coordinates_migrated() ) {
			return self::get_coordinates_new( $address, $skip_cache );
		}
	 ...
	 *
	 * @since 1.5
	 * @since 2.0 Tries to do less stuff. Namely, does not update cache, just returns data from API.
	 *
	 * @param string $address
	 * @param string|null $address_hash
	 * @param array|null $coordinates_set
	 *
	 * @return mixed|string|void
	 */
	protected static function get_coordinates_azure( $address, $address_hash=null, $coordinates_set=null ) {
		if ( self::$api_call_throttle >= self::API_CALL_THROTTLE_MAX ) {
			return self::get_api_throttling_reached_message();
		} else {
			self::$api_call_throttle++;
		}
		...
		if ( $response['response']['code'] == 200 ) {
			...
			if ( $address_hash === null && $coordinates_set === null ) {
				$data = [
					'lat' => $result->position->lat,
					'lon' => $result->position->lon,
					'address' => (string) $result->address->freeformAddress,
					'address_passed' => $address,
				];
			} else {
	 ...
	 */
	protected static function get_coordinates_google( $address, $address_hash=null, $coordinates_set=null ) {
		if ( self::$api_call_throttle >= self::API_CALL_THROTTLE_MAX ) {
			return self::get_api_throttling_reached_message();
		} else {
			self::$api_call_throttle++;
		...
		if ( $response['response']['code'] == 200 ) {
			...
			if ( $data->status === 'OK' ) {
				...
				if ( $address_hash === null && $coordinates_set === null ) {
					...
					$address_coordinates = $coordinates_set[ $address_hash ];
